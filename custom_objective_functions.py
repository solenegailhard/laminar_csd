...
def _rmse_evoked(initial_net, initial_params, set_params, predicted_params,
                 update_params, obj_values, tstop, n_trials, obj_fun_kwargs):
    ...
    n_trials : float
	The number of trials for the optimization
    ...
    obj = _rmse(avg_dpl, obj_fun_kwargs['target'], tstop=tstop)
...
def _maximize_psd(initial_net, initial_params, set_params, predicted_params,
    ...
    The objective function can be represented as -?c[?PSD(i)/?PSD(j)] where c
